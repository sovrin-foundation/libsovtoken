...
EOF
    ...
    mv target/${CROSS_COMPILE}/release/libindy.so target/${CROSS_COMPILE}/
    mv target/${CROSS_COMPILE}/release/libindy.a target/${CROSS_COMPILE}/
    command popd > /dev/null
done

BUILD_TIME=$(date -u "+%Y%m%d%H%M")
GIT_REV=$(git --git-dir indy-sdk/.git rev-parse --short HEAD)
command pushd indy-sdk/libindy/target > /dev/null
zip "libindy_${BUILD_TIME}-${GIT_REV}_android.zip" `find . -type f \( -name "libindy.so" -o -name "libindy.a" \) -not -path "./*/deps/*"`
command popd > /dev/null
