...
pub fn build_and_submit_cred_def_with_fees() {
    ...
    let (parsed_resp, cred_def_id, _) = _send_cred_def_with_fees(dids[0], rand_string(5).as_str(), SCHEMA_VERSION, GVT_SCHEMA_ATTRIBUTES, wallet.handle, pool_handle, &inputs, &outputs, None, None).unwrap();
...
pub fn build_and_submit_cred_def_with_fees_insufficient_funds() {
    ...
    let parsed_err = _send_cred_def_with_fees(dids[0], rand_string(3).as_str(), SCHEMA_VERSION, GVT_SCHEMA_ATTRIBUTES, wallet.handle, pool_handle, &inputs, &outputs, None, None).unwrap_err();
...
pub fn build_and_submit_cred_def_with_fees_double_spend() {
    ...
    _send_cred_def_with_fees(dids[0], rand_string(3).as_str(), SCHEMA_VERSION, GVT_SCHEMA_ATTRIBUTES, wallet.handle, pool_handle, &inputs, &outputs, None, None).unwrap();
...
pub fn build_and_submit_cred_def_with_fees_twice_and_check_utxo_remain_unspent() {
    ...
    let schema = Some(_create_schema_json(dids[0], rand_string(3).as_str(), SCHEMA_VERSION, GVT_SCHEMA_ATTRIBUTES, wallet.handle, pool_handle));
    let parsed_err = _send_cred_def_with_fees(&did_new, rand_string(3).as_str(), SCHEMA_VERSION, GVT_SCHEMA_ATTRIBUTES, wallet.handle, pool_handle, &inputs, &outputs, None, schema).unwrap_err();
...
fn _send_cred_def_with_fees(did: &str,
    ...
    let schema = schema.unwrap_or_else(|| _create_schema_json(did, name, version, attrs, wallet_handle, pool_handle));
...
fn _create_schema_json(did: &str,
                       name: &str,
                       version: &str,
                       attrs: &str,
                       wallet_handle: i32,
