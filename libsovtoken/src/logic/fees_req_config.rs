...
impl FeesRequest {
     ...
     */
    pub fn new(outputs: Vec<Output>, inputs: Vec<Input>) -> Request<FeesRequest> {
        let fees = FeesRequest {
            txn_type: "10000",
            inputs,
            outputs,
        };
    ...
    pub fn from_config(fees_output_config: OutputConfig, fees_input_config: InputConfig) -> Request<FeesRequest> {
        return FeesRequest::new(fees_output_config.outputs, fees_input_config.inputs );
    }
}
...
mod fees_req_input_config_test {
    ...
    fn serializing_fee_struct_output_config() {
        ...
        let input = Input::new(String::from("dakjhe238yad"),15,String::from("239asdkj3298uadkljasd98u234ijasdlkj"), None);
...
mod fees_request_test {
    ...
    fn initial_fees_request() -> Request<FeesRequest> {
        let output = Output::new(String::from("AesjahdahudgaiuNotARealAKeyygigfuigraiudgfasfhja"), 10, None);
        let input = Input::new(String::from("dakjhe238yad"),15,String::from("239asdkj3298uadkljasd98u234ijasdlkj"), None);

        let outputs = vec![output];
        let inputs = vec![input];
        return FeesRequest::new(outputs, inputs);
    }

    fn assert_fees_request<F>(expected: serde_json::Value, f: F)
        where F: Fn(&mut Request<FeesRequest>) -> ()
    {
        ...
        let deserialized_fees_request: Request<FeesRequest> = Request::<FeesRequest>::from_json(fees_req_json_str).unwrap();
    ...
    fn create_request_with_fees_config() {
        ...
        let fees_output_config = OutputConfig {
            outputs: outputs.clone()
        };

        let fees_input_config = InputConfig {
            inputs: inputs.clone()
        };

        let request = FeesRequest::from_config(fees_output_config, fees_input_config);
    ...
    fn valid_request() {
        assert_fees_request(
            json!({
                ...
                "inputs": [["dakjhe238yad", 15, "239asdkj3298uadkljasd98u234ijasdlkj"]]
            }),
            |_fees_req| {}
        )
    }
